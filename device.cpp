...
void Device::draw(QPainter &painter){
    ...
    QPixmap pix = QPixmap(imgPath).transformed(QTransform().rotate(90*getDirect()));
    if(selected){
        painter.setPen(QPen(QColor(selectR, selectG, selectB, 120), 4, Qt::SolidLine));
        painter.setBrush(QBrush(QColor(selectR, selectG, selectB, 60), Qt::SolidPattern));
        painter.drawRect(BaseX+x*PX, BaseY+y*PX, w*PX, h*PX);
    }
    painter.drawPixmap(QRect(BaseX+x*PX+MA, BaseY+y*PX+MA, w*PX-2*MA, h*PX-2*MA),pix);
    painter.save();
}
